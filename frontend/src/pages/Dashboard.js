...
export default function Dashboard() {
  ...
  const currentProfessions = PROFESSIONS.slice(page * ITEMS_PER_PAGE, (page + 1) * ITEMS_PER_PAGE);
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <header className="mb-12 flex justify-between items-end px-2">
          <div>
            <h1 className="text-4xl md:text-5xl font-black text-[#1e293b] tracking-tight italic">
              Bienvenid@, <span className="opacity-70">Usuario</span>
            </h1>
          ...
          <div className="flex gap-3">
            <button
              onClick={() => setPage(0)}
              className={`p-4 rounded-full border border-slate-300 transition-all shadow-lg active:scale-90 ${page === 0 ? "bg-[#1e293b] text-white border-[#1e293b]" : "bg-white text-[#1e293b] hover:bg-slate-50"}`}
            >
              <ArrowLeft className="w-5 h-5" />
            </button>
            <button
              onClick={() => setPage(1)}
              className={`p-4 rounded-full border border-slate-300 transition-all shadow-lg active:scale-90 ${page === 1 ? "bg-[#1e293b] text-white border-[#1e293b]" : "bg-white text-[#1e293b] hover:bg-slate-50"}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 animate-in fade-in duration-500">
          {currentProfessions.map((job) => (
            ...
            >
              <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 shadow-inner transition-colors ${selectedId === job.id ? "bg-white/20 text-white" : "bg-slate-100 text-[#1e293b] group-hover:bg-[#1e293b] group-hover:text-white"}`}>
                {job.icon}
              </div>
              <h3 className={`text-xl font-black mb-1 ${selectedId === job.id ? "text-white" : "text-[#1e293b]"}`}>
                {job.title}
              </h3>
              <p className={`text-[10px] font-black uppercase tracking-widest mb-4 ${selectedId === job.id ? "text-white/60" : "text-slate-400 font-bold"}`}>
                {job.area}
              </p>
              <p className={`text-sm leading-relaxed font-bold italic ${selectedId === job.id ? "text-white/80" : "text-slate-600 opacity-80"}`}>
                {job.desc}
              </p>
            </div>
          ))}
